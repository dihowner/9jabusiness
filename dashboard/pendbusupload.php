...
<head>
  ...
  <title>9jaBusiness - Pending Business Upload</title>
  <!-- loader--
...
<div class="clearfix"></div>
    ...
    <div class="row mt-3">
      <div class="col-lg-12">
         <div class="card">
			<div class="card-body">
				<div class="card-header" style="font-size: 20px">Pending Business Upload</div>
				...
				if(isset($_REQUEST["viewBus"])) {
					...
					//Calculation for all input...
					// $points = array();
					
					// if(!empty($loadBuss["businessname"])) { $points[] = $action->businesspoint()->creation; }
					// if(!empty($loadBuss["url"])) { $points[] = $action->businesspoint()->creation; }
					// if(!empty($loadBuss["founder"])) { $points[] = $action->businesspoint()->creation; }
					// if(!empty($loadBuss["mobile"])) { $points[] = $action->businesspoint()->creation; }
					// if(!empty($loadBuss["datefounded"])) { $points[] = $action->businesspoint()->creation; }
					// if(!empty($loadBuss["workhours"])) { $points[] = $action->businesspoint()->creation; }
					// if(!empty($loadBuss["address"])) { $points[] = $action->businesspoint()->creation; }
					// if(!empty($loadBuss["description"])) { $points[] = $action->businesspoint()->creation; }
					// if(!empty($logo)) { $points[] = $action->businesspoint()->creation; }
					// if(!empty($office_pic)) { $points[] = $action->businesspoint()->creation; }
					
					// print_r($points);
					// $totalPoint = array_sum($points);
					...
					<div class="row" style="margin-top: 10px">
						<div class="col-lg-12">
							...
							<textarea class="form-control" id='dclnMsg' name='dclnMsg' placeholder="Reason for declining this business"></textarea>
						</div>
					</div>
					...
					<script>
						function apprBus(ev) {
							...
							if(totalPoint == "") {
								...
								swal.fire({
									title: "Are you sure?",
									html: "You are about to approve this business (<b>"+businessname+"</b>), a total of <b>"+totalPoint+"</b> USD will be credited to this user",
									icon: "warning",
									buttons: true,
									dangerMode: true,
				...
				<?php } else if(isset($_REQUEST["txType"])) {
					...
					switch($_REQUEST["txType"]) {
						...
						case "approve":
							...
							$loadBus = $action->query("select * from businesscreation where id='$payID'"); $loadBus->execute();
							...
							if($loadBus->rowCount() > 0) {
								...
								if($loadBuss["status"] == 1) { ?>
									<script>
										swal.fire({ html: "<font size='5px'>Business already approved</font>", icon: "error"})
										.then((isRedirect) => {
									
											if(isRedirect.isConfirmed) {
												window.location = 'pendbusupload'
											}
								...
								<?php } else if($loadBuss["status"] == 0) {
									...
									$response = json_encode([
										"msg" => "Approval of Business creation",
										"bus_id" => $payID,
										"ngnAmnt" => $point,
									...
									$updtBus = $action->query("update businesscreation set status=1, apprBy='".$userInfo["fullname"]."', earnings='$totalPoint', dateApproved='".date("D j F, Y; h:i a")."' where id='$payID'"); $updtBus->execute();
									...
									$message = "<html>
            <head>
                <title>Approval of Business Creation</title>
            </head>
        ...
        <table border='0' cellpadding='0' cellspacing='0' width='100%' style='max-width: 600px;'>
          <tr>
            <td align='left' bgcolor='#ffffff' style='padding: 36px 24px 0; font-family: Helvetica, Arial, sans-serif; border-top: 3px solid #264762;'>
              ...
              <h4 style='text-align: center; margin: 0; font-size: 20px; font-weight: 700; letter-spacing: -1px; line-height: 48px;'>Approval of Business Creation!</h4>
            </td>
          </tr>
        </table>
      </td>
        ...
        <table border='0' cellpadding='0' cellspacing='0' width='100%' style='max-width: 600px;'>
          <tr>
            <td align='left' bgcolor='#f5a21b' style='padding: 24px; font-family: Helvetica, Arial, sans-serif; font-size: 16px; line-height: 24px;'>
              <p style='margin: 0;'>
				Hey <b>".$action->clientInfo($loadBuss["userid"])['fullname'].", </b> <br/> Your business creation for ".$loadBuss["businessname"]." has been approved successfully and <b>$".number_format($totalPoint, 2)." has been credited to your wallet
...
$action->sendmail("Approved Business Creation", "no-reply@9jabusiness.com", $action->clientInfo($loadBuss["userid"])['email'], $message);	
									
									
								?>
									<script>
										swal.fire({ html: "<font size='5px'>Business approved successfully</font>", icon: "success"})
										.then((isRedirect) => {
								...
								<?php 
									...
									<script>
										swal.fire({ html: "<font size='5px'>Business already declined</font>", icon: "error"})
										.then((isRedirect) => {
						...
						case "decline" :
							...
							if($loadBus->rowCount() > 0) {
								...
								<?php 
									...
									$txid = mt_rand(11111, 99999).mt_rand(11111, 99999);
									$response = json_encode([
										"msg" => "Declination of Business creation",
										"bus_id" => $payID,
										"type" => "creation"
									]);
									
									$updtBus = $action->query("update businesscreation set status=2, msg='$declnMsg', apprBy='".$userInfo["fullname"]."', dateApproved='".date("D j F, Y; h:i a")."' where id='$payID'"); $updtBus->execute();
									...
									$message = "<html>
            <head>
                <title>Declined Business Creation</title>
            </head>
			<body style='background-color: #030f4f;'>
  <table border='0' cellpadding='0' cellspacing='0' width='100%' style='margin-top: 2%'>
    <tr>
      <td align='center' bgcolor='#030f4f'>
        <table border='0' cellpadding='0' cellspacing='0' width='100%' style='max-width: 600px;'>
          <tr>
            <td align='left' bgcolor='#ffffff' style='padding: 36px 24px 0; font-family: Helvetica, Arial, sans-serif; border-top: 3px solid #264762;'>
              <h1 style='text-align: center; margin: 0; font-size: 36px; font-weight: 700; letter-spacing: -1px; line-height: 48px;'>9ja Business</h1>
              <h4 style='text-align: center; margin: 0; font-size: 20px; font-weight: 700; letter-spacing: -1px; line-height: 48px;'>Declined Business Creation!</h4>
            </td>
          </tr>
        </table>
      </td>
      ...
      <td align='center' bgcolor='#030f4f'>
        <table border='0' cellpadding='0' cellspacing='0' width='100%' style='max-width: 600px;'>
          <tr>
            <td align='left' bgcolor='#f5a21b' style='padding: 24px; font-family: Helvetica, Arial, sans-serif; font-size: 16px; line-height: 24px;'>
              <p style='margin: 0;'>
				Hey <b>".$action->clientInfo($loadBuss["userid"])['fullname'].", </b> <br/> Your business creation for ".$loadBuss["businessname"]." has been declined successfully
...
$action->sendmail("Declined Business Creation", "no-reply@9jabusiness.com", $action->clientInfo($loadBuss["userid"])['email'], $message);	
								 
								 ?>
									<script>
										swal.fire({ html: "<font size='5px'>Business declined successfully</font>", icon: "success"})
										.then((isRedirect) => {
				...
				<div class="table-responsive">
					<table class="table table-bordered">
						...
						<tr>
							<?php $i = 1;
							$loadBus = $action->query("select * from businesscreation where status=0 order by id desc limit $start_page, $per_page"); $loadBus->execute();
							...
							<td>
								...
								<b style="color: #f5a21b">Date Created : </b> <?php echo $loadBuss["dateCreated"];?></b> <br>
							</td>
							<td>
					...
					<div class="text-center" style="margin-top: 2%">
						<?php echo $action->paginate("", "select * from businesscreation where status=0 order by id desc");?>
					</div>
				</div>
