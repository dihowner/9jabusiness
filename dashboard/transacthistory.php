...
<head>
  ...
  <title>9jaBusiness - Transaction History</title>
  <!-- loader--
           ...
           <hr>
			<div class="table-responsive">
				<table class="table table-bordered table-striped">
					...
					<tr>
					 <?php $i = 1;
					   $loadTran = $action->query("select * from transaction order by id desc limit $start_page, $per_page");
					   $loadTran->execute();
					   while($loadTrans = $loadTran->fetch(PDO::FETCH_ASSOC)) {
							...
							} else if(strpos(strtolower($response->msg), "earn") !== FALSE) { //Earnings made
								$amount = "<b style='color: #59b259'>&dollar;".number_format($loadTrans["amount"], 2)."</b>";
							} else {
								$amount = "<b style='color: #f75800'>&dollar;".number_format($loadTrans["amount"], 2)."</b>";
							}
							
							if($loadTrans["type"] == "withdraw") {
								...
								if($loadTrans["status"] == 0) {
									...
									if(strpos(strtolower($response->msg), "declin") !== FALSE) {
										$gStat = "<b style='color: #f5a21b'>Business Declined</b>";
									} else {
										$gStat = "<b style='color: #f5a21b'>Activation Failed</b>";
									}
								} else if($loadTrans["status"] == 1) {
									if(strpos(strtolower($response->msg), "approval") !== FALSE) {
										$gStat = "<b style='color: #e9a84c'>Business Approved</b>";
									} else if(strpos(strtolower($response->msg), "review") !== FALSE) {
										$gStat = "<b style='color: #e9a84c'>Review Approved</b>";
									} else {
					   ...
					   ?>
						   ...
						   <?php
							...
							<td>
								<?php echo $loadTrans["dateCreated"] . '<br><b style="color: #f5a21b">Status : </b>'. $gStat;
								if($loadTrans["status"] == 1) { ?>
									<br><b style="color: #e9a84c">Approved By : </b> <?php echo $loadTrans["apprBy"];?>
								<?php } else { ?>
									<br><b style="color: #E4CD05">Declined By : </b> <?php echo $loadTrans["apprBy"];?>
								<?php } ?>
							</td>
							<td><?php echo $loadTrans["dateUpdated"];?></td>
					   </tr>
					   <?php } ?>
				</table>
				
				<div class="text-center" style="margin-top: 2%; margin-bottom: 2%; ">
					<?php echo $action->paginate("", "select * from transaction order by id desc");?>
				</div>
