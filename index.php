...
<li> Click on Review Business and comment at the bottom of the business details.</li>
    ...
    <section id="services" class="contact">
      <div class="container" data-aos="fade-up">
        ...
        <div class="row">
			<?php
			$srchWith = $action->query("select * from withdrawal where status=1 ORDER by id DESC LIMIT 6");
			// $srchWith = $action->query("select * from withdrawal ORDER by id DESC LIMIT 8"); 
			$srchWith->execute();
			while($srchWiths = $srchWith->fetch(PDO::FETCH_ASSOC)) {
				$clientID = $srchWiths["userid"];
				if($srchWiths["type"] == 'bank') {
					$amnt = "&#8358;".number_format($srchWiths["amount"], 2);
					$icon = 'bx bx-money';
				} else {
					$amnt = number_format($srchWiths["amount"], 2)."EDT";
					$icon = 'bx bx-coin';
				}
				?>
				...
				<div class="col-lg-3 col-md-6">
					<div class="info-box  mb-4">
						...
						<h3><?php echo $amnt;?></h3>
						<h4><?php echo ucfirst($action->loggedusername($srchWiths["userid"]));?></h4>
						<p><b style="color: red">Payout Date:</b><br><?php echo $srchWiths["dateApproved"];?></p>
					</div>
				</div>
